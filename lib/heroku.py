    ...
    if not compact:
        ...
        if sp["incidents"] == []:
            ...
            for actinc in cinc:
                ...
                print(
                    colored(" Last comment: ", attrs=["bold"])
                    + actinc["updates"][0]["contents"]
                )

        print()
        print(
            colored("Heroku", "magenta", attrs=["bold"]) + " incidents in the past day:"
        )
        print("-------------------------")
        ...
        if ipr == []:
            ...
            for inc in ipr:
                if inc["state"] != "open":
                    ...
                    print(
                        colored(" Initial comment: ", attrs=["bold"])
                        + inc["updates"][-1]["contents"]
                    )
                    print(
                        colored(" Final comment: ", attrs=["bold"])
                        + inc["updates"][0]["contents"]
                    )
                    print()
        print()
