...
def get_heroku_status(heroku_api, compact, time, today, yesterday):
    ...
    print(colored('Heroku', 'magenta', attrs=[
          'bold']) + ' Status at ' + time + ':')
    print('-------------------------')
    ...
    if not compact:
        print('Current ' + colored('Heroku', 'magenta',
                                   attrs=['bold']) + ' incidents:')
        print('-------------------------')
        ...
        if sp['incidents'] == []:
            ...
            for actinc in cinc:
                ...
                print(colored(' Incident ID: ', attrs=[
                      'bold']) + str(actinc['id']))
                print(colored(' State: ', attrs=['bold']) + actinc['state'])
                print(colored(' More info: ', attrs=[
                      'bold']) + actinc['full_url'])
                print(colored(' Last comment: ', attrs=[
                      'bold']) + actinc['updates'][0]['contents'])

        print()
        print(colored('Heroku', 'magenta', attrs=[
              'bold']) + ' incidents in the past day:')
        print('-------------------------')

        ir = requests.get(
            '{}/incidents?since={}&per_page=3'.format(heroku_api, yesterday))
        ...
        if ipr == []:
            ...
            for inc in ipr:
                if inc['state'] != 'open':
                    ...
                    print(colored(' More info: ', attrs=[
                          'bold']) + inc['full_url'])
                    print(colored(' Created at: ', attrs=[
                          'bold']) + inc['created_at'])
                    print(colored(' Initial comment: ', attrs=[
                          'bold']) + inc['updates'][-1]['contents'])
                    print(colored(' Final comment: ', attrs=[
                          'bold']) + inc['updates'][0]['contents'])
                    print()
        print()
