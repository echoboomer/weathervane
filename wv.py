...
parser = argparse.ArgumentParser(
    description='Gather diagnostic information from third party providers for use during incidents and diagnostic operations.')
parser.add_argument('-p', '--providers', dest='providers', type=str,
                    help='Comma separated list of providers to get reports from. Options are: gcp, github, heroku', default='')
parser.add_argument('-c', '--compact', dest='compact',
                    help='Show compact summary instead of verbose output. Ignores incident details and header.', action='store_true')
...
def run():
    ...
    if providers == '':
        print(colored('Error: ', 'red') +
              ' No providers supplied. Please use -p to declare providers to check. Try -h for help.')
